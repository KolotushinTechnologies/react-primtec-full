...
export const getProduct = (keyword = "", currentPage = 1, cityState, category) => async (dispatch) => {
  try {
    dispatch({
    ...
    let link = `/api/v2/products?city=${cityState}&keyword=${keyword}&page=${currentPage}`;

    if (category) {
      link = `/api/v2/products?city=${cityState}&keyword=${keyword}&page=${currentPage}&category=${category}`;
    }
...
export const getProductByCity = (city) => async (dispatch) => {
  try {
    dispatch({
      type: ALL_PRODUCT_REQUEST
    });
    ...
    let link = `/api/v2/products?city=${city.toString()}`;
